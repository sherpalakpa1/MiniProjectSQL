...
SELECT COUNT(name) AS count_facilities_nocharge
FROM Facilities
WHERE membercost = 0
...
FROM Facilities
WHERE membercost < (0.2*montlymaintenance) AND membercost !=0
...
ELSE 'cheap' END AS budget, name, monthlymaintenance
FROM Facilities
...
OR CASE WHEN b.memid = 0 THEN f.guestcost*b.slots
ELSE f.membercost*b.slots END >30)
ORDER BY cost DESC
